...
variable "id_01_tailnet_ipv4" {
  description = "Tailscale IPv4 for id-01. DNS-only; public clients cannot reach it without the tailnet."
  type        = string
...
resource "cloudflare_dns_record" "id_tailnet" {
  provider = cloudflare.zone
  zone_id  = var.cloudflare_zone_id
  name     = "id.${local.domain}"
  type     = "A"
  content  = var.id_01_tailnet_ipv4
  ttl      = 1
  proxied  = false
...
output "id_tailnet" {
  value = {
    name    = cloudflare_dns_record.id_tailnet.name
